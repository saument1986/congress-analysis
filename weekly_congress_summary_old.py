...
def is_important_bill(bill):
    """Filter for bills that actually affect people and cost money"""
    title = bill['title'].lower()
    
    # Skip naming, commemorative, and ceremonial bills
    skip_words = [
    ...
    # Focus on bills that cost money or affect people's lives
    important_words = [
        'appropriation', 'budget', 'tax', 'spending', 'fund',  # Money bills
        'healthcare', 'medicare', 'medicaid', 'insurance',     # Health
        'social security', 'veterans', 'benefits',            # Benefits
        'infrastructure', 'transportation', 'energy',         # Infrastructure
        'immigration', 'border', 'security',                  # Security/Immigration
        'education', 'student loan', 'college',               # Education
        'housing', 'mortgage', 'rent',                        # Housing
        'employment', 'minimum wage', 'jobs',                 # Work/Jobs
        'environment', 'climate', 'pollution',                # Environment
        'reform', 'act', 'authorization'                      # Major legislation
    ]
...
def analyze_with_fabric(bill_text):
    """Analyze bill text with fabric using Claude"""
    try:
        # Save to temp file to handle large bills
        temp_file = '/tmp/bill_analysis.txt'
        with open(temp_file, 'w', encoding='utf-8') as f:
            f.write(bill_text)
        
        # Use fabric with Claude (now default)
        cmd = f"cat {temp_file} | {FABRIC_PATH} --pattern extract_wisdom"
